defmodule LexOffice.CreditNotes do
  @moduledoc """
  API calls for all endpoints tagged `CreditNotes`.
  """
  ...
  """
  @spec create(Tesla.Env.client(), LexOffice.Model.CreateInvoice.t(), boolean(), keyword()) ::
          {:ok, LexOffice.Model.CreateInvoiceResponse.t()} | {:error, Tesla.Env.t()}
  def create(connection, invoice, finalize \\ false, _opts \\ []) do
    ...
    %{}
    |> method(:post)
    |> url("/v1/credit-notes#{url_append}")
  ...
  {:error, info} on failure
  """
  @spec get(Tesla.Env.client(), String.t()) ::
